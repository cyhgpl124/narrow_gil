...
class BentoItem extends Equatable {
  ...
  BentoItem copyWith({
    String? id, String? title, String? route, double? x, double? y, double? width, double? height,
  }) {
    return BentoItem(
      id: id ?? this.id,
  ...
  Map<String, dynamic> toFirestore() {
    return {
      'title': title, 'route': route, 'x': x, 'y': y, 'width': width, 'height': height,
    };
  }
  ...
  static List<BentoItem> get defaultItems {
    ...
    return [
      const BentoItem(id: 'my_page', title: '마이페이지', route: '/my_page', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'attendance_check', title: '출석체크', route: '/attendance_check', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'new_life', title: '신생활', route: '/new_life', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'gallery', title: '밀알스타그램', route: '/gallery', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'forum', title: '제직회의', route: '/forum', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'schedule', title: '일정', route: '/schedule', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'announcements', title: '공지', route: '/announcements', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'bible', title: '말씀필사', route: '/bible', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'accounting', title: '회계', route: '/accounting', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'events', title: '행사', route: '/events', x: 0, y: 0, width: 0, height: 0),
      const BentoItem(id: 'questions', title: '질문', route: '/questions', x: 0, y: 0, width: 0, height: 0),
  ...
  static List<BentoItem> applyGridAutoLayout(List<BentoItem> items) {
    ...
    // 최종 아이템 목록 (교인 관리 포함)
    final finalItems = [...otherItems, if (memberManagementItem != null) memberManagementItem];
    ...
    final requiredHeight = (rowCount * itemHeight) + ((rowCount + 1) * verticalGap);
